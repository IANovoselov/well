...
class Oil:
    ...
    gamma = (1 - betta) * gamma_oil + betta * gamma_water  # Удельный вес флюида
...
class Reservoir:
    ...
    def __init__(self, w_1, w_2, p_R):
        # Гидросопротивления
        self.w_1 = w_1  # Гидропроводность ПЗ - забой
        self.w_2 = w_2  # Гидропроводность Пласт - ПЗ
        self.r_1 = 1 / self.w_1  # ПЗ - забой
        self.r_2 = 1 / self.w_2  # Пласт - ПЗ
        self.r_R = self.r_1 + self.r_2
        self.p_R = p_R  # Среднепластовое давление
...
class Pump:
    ...
    def set_w_by_program(self, key, dt):
        ...
        if self.smooth_enable:
            self.w = transfer_function(self.w, self._w, dt * 300, dt)
        else:
            self.w = self._w
    ...
    def calc_h_N(self):
        h_N = self.h_0 * (
            self.w * self.w * self.lambda_0[self.i] - self.w * self.lambda_1[self.i] * (self.q_N / self.q_0)
        )
    ...
    def calc_N(self, oil):
        ...
        self.N_1 = (
            ...
            * (1 + delta_n_N)
            * (
                self.w * self.w * self.w * self.lambda_2[self.i]
                + self.w * self.w * self.lambda_3[self.i] * (self.q_N / self.q_0)
            )
        )
    ...
    def _init_q_N(self, oil, params, reservoir):
        return max(
            (self.w * self.w * self.h_0 * self.lambda_0[self.i] - params.H_R + (1 / oil.gamma) * (reservoir.p_R - p_L))
            / (
                self.w * (self.h_0 / self.q_0) * self.lambda_1[self.i]
                + params.r_U
                + self.r_N
                + params.r_K
                + (reservoir.r_R / oil.gamma)
            ),
            0,
        )
...
class AGZU:
    ...
    def process(self, _k, dt, p_L_avg, p_L_change_enable):
        ...
        if _k > 0 and _k % (self.PERIOD_TIME/dt) == 0:
...
class Well:
    ...
    def __init__(self, oil: Oil, pump: Pump, reservoir: Reservoir, well_params: WellParameters, agzu: AGZU):
        ...
        self.p_4 = (
            ...
            * self.params.a_GU
            * np.sqrt(self.pump.w)
            * max((self.params.p_G0 - self.p_3), 0)
            * self.q
        )
    ...
    def calc_w(self, q_plan):
        ...
        c = -(
            ((self.params.r_U + self.pump.r_N + self.params.r_K + (self.reservoir.r_R / self.oil.gamma)) * q_plan)
            - self.params.dH_R
        )
